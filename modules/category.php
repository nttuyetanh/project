...
<div class="container py-5">
    <div class="row">
        <div class="col-lg-12">
            ...
            <?php 
                $cat_id = $_GET['cat_id'];
                $query_select_category = "SELECT * FROM tbl_category WHERE cat_id = '$cat_id'";
                $result_select_category = $conn->query($query_select_category);
                $row_select_category = $result_select_category->fetch();
            ?>
            <h6>Shop / <?php echo $row_select_category['cat_name'] ?></h6>
            <div class="row">
                <?php
                    $product = $conn->query("SELECT * FROM tbl_product WHERE `status`=1 AND cat_id ='$cat_id'");
                    ...
                    // echo $trang_index;
                    // die();
                    $sqlPro = "SELECT * FROM tbl_product WHERE `status`=1 AND cat_id ='$cat_id' LIMIT $start_in,$limit";
